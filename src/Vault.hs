...
ffOrder (Vault threshold shares secret') = nextPrime $ max (C.ord secret') shares
...
generateShare vault@(Vault threshold shares secret') = do
         ...
         case someNat of
             TL.SomeNat (_ :: Proxy n1) -> do
                 ...
                 let secretInt   = C.ord secret'
                 --let secret = Modp (C.digitToInt secret') :: Modp n1
                 let secret = Modp secretInt :: Modp n1
...
decrypt shares = do
         ...
         case someNat of
             TL.SomeNat (_ :: Proxy n1) -> do
                 ...
                 return P.$ C.chr (unMod (coeff 1 x y))
